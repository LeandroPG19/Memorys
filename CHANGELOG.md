...
- **`/health` never answers 503 again, and `status` is now three-valued** (`ok` / `starting` / `degraded`). Anything alerting on the status code stops firing — read `status` instead. A 503 takes a daemon out of rotation that still serves lexical search and still serves the one endpoint that can explain what is wrong; there is nothing here for it to be taken out of anyway, since this is a single daemon on a LAN. `starting` does not merit one either: that is the refused connection B8 just removed, one layer up.
- **With a token, `/health` carries a `runtime` block**: `mode`, `resource_tier`, per-model `{state, device, reason}` for the embedder, reranker and NLI, `gpu{build, degraded, placement}` and `llm{configured, backend, model, base_url}`. This is the question the operator of the first LAN deployment had to answer by reading the source — where is the reranker actually running. Never a key, never `DATABASE_URL`, never a model path (a path is inventory and usually carries a user name); the model *name* yes, and the base URL redacted with everything after `?` cut. An embedder on its CPU fallback is not `degraded`: a machine with no model is a supported tier, and marking it degraded forever teaches the operator to ignore the field.
...
- The gate's release binary keeps its GPU. `build-gpu.sh` built it with `--features cuda` and the next step ran `cargo test --release` without it into the same target directory; cargo fingerprints the feature set, so that call relinked the binary the 25-tool E2E then used. Measured: `doctor` reports `ok, cuda` after the build and `warn, compiled without support` after the test call — which ran with `-- --list` and never executed a test. Compiling them without the feature was enough. `v017_rerank_gpu` was compiled the same way, so the test whose name says GPU had never entered a `#[cfg(feature = "cuda")]` branch; it passed because the reranker also works on the CPU. A guard now extracts every `--release` cargo invocation in the gate and requires its feature set to equal the one `build-gpu.sh` uses — with the fix reverted the file still contains `--features cuda` on four lines, so asking whether the string is present would have stayed green.
...
The theme of this branch. Each of these ran green for as long as it existed and proved nothing.

- **`doctor` never once reported a stale daemon.** `stale_processes` filtered `/proc/*/exe` on the file name being exactly `cuba-memorys`, and no unit this repo packages produces that name: the current one starts `memory-industry` and the legacy one starts `cuba-memorys-daemon`. The filter dropped every process, so the warning that tells an operator the daemon is still serving the binary they replaced has never been able to fire on a systemd install. Its only test re-implemented the `strip_suffix(" (deleted)")` inside its own body and asserted on that copy — deleting the filter outright left it green. The decision is now a pure function the tests call, the three names are accepted, and the ` (deleted)` suffix comes off the path that is handed back rather than merely being detected: that path is what the caller then stats.
- **The two-pass protocol had no guard.** `validar-handoff.sh` checked the shape of a YAML and nothing else; `commit` only had to look hexadecimal. It now requires the commit to exist and a `tests: written|frozen` field, and compares the `#[cfg(test)]` regions — and whole files under `rust/tests/`, 78 of which carry no such attribute — against it. Two-way: `written` fails when no test moved, `frozen` fails when one did. Run over the 31 handoffs in the tree, two came back red, and both were green passes that the old guard had waved through. Its own "missing field" branch had never printed in its life: with `set -e`, `val()` died inside the assignment and the script exited 1 in silence.
- **The complexity baseline had a silent ceiling.** `judge.rs` holds two `run_prompt`, the lookup took the first in file order, and the CC 9 one sat behind the other's ceiling of 10. The key now carries the start line, which is unique by construction; the cost is that moving a function invalidates its key, and that failure is loud instead of silent.
- **The `TimeoutLayer` had no test that saw a 504.** It has one now, and it costs eight seconds of wall clock.
- **Five test files had three different ways to wait for a daemon**, two of which could only ever be slow, never red: a fixed `sleep` and a client with no timeout means the request waits in the backlog. One form now, with a per-probe timeout and an assertion that the daemon is still alive.

- **The CLI contract accepted anything that did not exit with 2.** `every_listed_command_is_actually_dispatched` could not tell "printed its help" from "tried to run and failed to connect", which is how `secure --help` could run DDL unseen, and how eleven `run_cli` replaced by `Ok(())` survived mutation with nothing anywhere to judge them. It now demands exit 0, a line beginning `memory-industry <cmd>`, and no connection to PostgreSQL.
- **Two gates on one machine could drop each other's database.** A gate orphaned by a dead session fired its `trap` in the middle of the next run; the failure read `database "brain_gate" does not exist`. `merge-gate.sh` now holds a lock (`scripts/gate-lock.sh`) for the whole run, a second gate refuses at once naming the first, and an exit drops only databases that still carry its own record.
- **Exclusion expiry dates were read by nothing.** The second judge now fails when an exclusion's date passes, when a line loses its date, or when a pattern matches no mutant in the crate.
- **A machine out of memory scored a perfect kill rate.** `mutants-gate.sh` printed `3 caught, 51 unviable … kill_rate=1.000` and the SIL passed; the good run of the same commit was 53 caught, 1 unviable. 50 of those 51 were the machine: 44 `cargo.exe` that ended with `0xC0000142 STATUS_DLL_INIT_FAILED`, and 6 that exited 101 like a compile error while their logs held no rustc diagnostic, only `could not compile` or `link.exe` dying of the same status. The kill rate leaves unviable mutants out of its denominator. An unviable mutant now counts only when its build exited 1..255 **and** its log shows rustc refusing it inside the build phase (an `error[E…]:` or `error:` line other than cargo's summary or a linker failure); anything else, a missing log included, fails the run naming the count. Over that run's `outcomes.json` it reports 50 of 51; the one left is `Some(Default::default())` on a `V` with no `Default` bound, E0277. `quality-gate.sh` hands its mutation outcomes to the same check, from a fresh output directory; `mutants-gate.sh --self-test` proves it refuses each case and still accepts a real compile error.
- **A contract loosened itself mid-branch.** Moving the gate's `trap` made the provisioning-order contract find a function's definition instead of its call; it stayed green. It now finds the call, and a test moves the call behind `cargo test` and asserts the contract refuses it.

### Fixed

- **`hook install` guards the directory sync actually writes to.** The command wires a repo's git so two machines merging the same knowledge graph do not clobber each other: a merge driver in `.git/config`, and a `.gitattributes` line saying which files it covers. `sync::paths` has resolved that directory as `.memory-industry` (falling back to `.cuba-memorys` only when it already exists) for some time, but `hooks_cli` built the pattern from a second, hand-written copy of the fallback still holding the old value. So on a fresh repo sync wrote into one directory and the driver guarded the other, `install` printed `installed`, and the first conflict between two machines was resolved by git's text merge over JSON instead of the union by id the whole command exists to arrange. The pattern is also made relative to the root, because git reads it relative to the file's directory and an absolute one matches nothing — fixing the name and leaving it absolute would have traded one defect for another. A configured root outside the repository still cannot be covered from here, which is a separate question.

- **`/health` can say a model failed.** `nli_state` had a `OnceLock<bool>` and the only thing that knew the truth was the call that loads a gigabyte; the embedder had no failed state at all, and both reasons died in a log line. Four outcomes used to collapse into `Fallback` and only one of them — the session that does not open — is a failure; a machine with no model directory is a supported tier and saying otherwise teaches the operator to ignore the field. The hot path does not move: a failed model still embeds with the hash, because the reason belongs in `/health` and not in the vector.
- **Eight GPU states read differently.** With no GPU feature compiled in, the provider libraries were reported absent by construction rather than by measurement, so an operator with a card could not tell whether the build was the last step or the first of two. Both halves are measured now, at the cost of two `Path::exists` calls and no new dependency. The runtime present with no card stays not degraded and keeps no hint: nothing is being wasted there and the one exit builds `--features cuda`.
- **The operator reads and types the name this release ships.** 57 sites across 24 files, never a global replace, because the same string spells a command, the product, and an identifier in somebody else's `.git/config`. The legacy cache paths, the sync directory, the merge driver, the hook marker, the npm package and the binary alias all stay.

### Hardened

- **`redact::scan` is a tokenizer again**, CC 22 down to 5, with five pure detectors. The 14 tests that hold it were written first, against the old body, and are byte-identical across the split. Five holes were closed before a line moved: four of the seven provider prefixes appeared in no test, a minimum-length constant decided nothing, the no-colon URL branch had no case, and the two views of the detector could drift. Two mutants that predated the split are dead — an assertion of absence that does not say what remains pins nothing — and the order guard that stands between user text and a reversed slice range now has a fixture that turns it red with a panic.
- **One place resolves the home.** Ten sites read `HOME` then `USERPROFILE` by hand; `grep` now returns one. Measured before it was touched: all ten read the same two names in the same order, and only the degradation differed — eight answered `None` in silence, which is correct for every one of them, so they take `.ok()`. The ten tests that pin today's behaviour were written and run green first, and not one was edited by the migration.
