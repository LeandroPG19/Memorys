...
| `CUBA_WARM_RERANKER` | **on** | Load the cross-encoder at startup instead of inside the first search. Opt-out now (`0`/`off`/`false`/`no`): deferring does not save the cost, it moves it into a request with a 20 s budget that has to pay for a 1.1 GB read |
